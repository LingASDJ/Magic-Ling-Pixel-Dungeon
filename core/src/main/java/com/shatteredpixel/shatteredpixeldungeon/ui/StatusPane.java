...
public class StatusPane extends Component {
	...
	protected void createChildren() {
		...
		hpText.hardlight(0x00ffff);
		add(hpText);

		exp = new Image( Assets.Interfaces.XP_BAR );
		add( exp );

		this.hg = new Image("interfaces/hunger_bar.png");
		add(this.hg);

		hgText = new BitmapText(PixelScene.pixelFont);
		hgText.hardlight(0x00ffff);
		add(hgText);
	...
	public void update() {
		...
		float f = 1.0f;
		PointF pointF = this.hg.scale;
		if (h != null) {
			f = (450.0f - ((float) h.hunger())) / 450.0f;
		}
		pointF.x = f;
