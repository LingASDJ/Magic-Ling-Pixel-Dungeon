...
public class AndroidPlatformSupport extends PlatformSupport {
	...
	public void updateDisplaySize(){
		...
		float renderWidth = dispRatio > 6 ? PixelScene.MIN_WIDTH_L : PixelScene.MIN_WIDTH_P;
		float renderHeight = dispRatio > 12 ? PixelScene.MIN_HEIGHT_L : PixelScene.MIN_HEIGHT_P;

		//force power saver in this case as all devices must run at at least 2x scale.
		if (Game.dispWidth < renderWidth*1 || Game.dispHeight < renderHeight*1)
		...
		if (SPDSettings.powerSaver() && fullscreen){
			...
			renderWidth *= Math.max( 2, Math.round(5f + maxZoom*0.4f));
			renderHeight *= Math.max( 12, Math.round(4f + maxZoom*0.1f));
